      ...
      <a-col :xs="24" :md="16">
        <a-image-preview-group>
          <div class="service-detail-page__cover">
            <a-image :src="coverImage" :alt="service.name" />
          </div>
          <div v-if="galleryThumbs.length" class="service-detail-page__thumbs">
            <div v-for="img in galleryThumbs" :key="img.id" class="service-detail-page__thumb">
...
// Pre-filled from a list page's ?startDate=&endDate= (Explore's
// detailsLink(), Hotels'/Transport's own links to this route) when present,
// otherwise empty so the user picks their own -- a-range-picker's v-model
// needs Dayjs instances, not plain ISO strings, hence the dayjs(...) wrap.
...
// This same component is mounted fresh for every id under /hotels/[id],
// /transport/[id] and /services/[id] -- but Vue Router reuses the component
// instance when only the dynamic param changes (e.g. navigating from one
// hotel's detail page straight to another's), so onMounted alone won't
// re-fetch. Watching route.params.id covers that in-place navigation case.
watch(() => route.params.id, fetchWithSelectedDates)
...
// ServicesService.findOne's allImagesInclude) -- coverImageFor() picks that
// same first image (or a type-appropriate default when this listing has no
// photos yet), so the banner always shows the image that actually belongs
// to *this* service/hotel/flight, never another module's or the homepage
// hero's art.
const coverImage = computed(() => coverImageFor(service.value))
const galleryThumbs = computed(() => service.value?.images?.slice(1) ?? [])
...
// way pages/hotels/index.vue's handleBookNow already does: populate
